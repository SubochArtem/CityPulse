...
namespace Users.Tests.Unit.Mapping;
...
public sealed class UserMappingConfigTests
{
    ...
    [Theory]
    [MemberData(nameof(AccessStatuses))]
    ...
    public void Map_PagedListOfUserToPagedListOfGetUserDto_MapsItemsAndPaginationFields()
    {
        ...
        Assert.Equal(users.Count, pagedDtos.Items.Count);

        for (var i = 0; i < users.Count; i++)
        {
            Assert.Equal(users[i].Id, pagedDtos.Items[i].Id);
            Assert.Equal(users[i].IdentityId, pagedDtos.Items[i].IdentityId);
            Assert.Equal(users[i].Nickname, pagedDtos.Items[i].Nickname);
            Assert.Equal(users[i].CityId, pagedDtos.Items[i].CityId);
            Assert.Equal((int)users[i].AccessStatus, pagedDtos.Items[i].AccessStatus);
        }
    }
